...
public class MainView extends VerticalLayout implements ComponentEventListener<ClickEvent<Button>> {
    ...
    public MainView() {
        ...
        spieler.addValueChangeListener(e -> name = e.getValue());
        add(spieler);
     ...
     *
     * @param parent @{@link VerticalLayout}
     */
    private void createBingoGrid(VerticalLayout parent) {
        ...
        for (int col = 0; col < numberOfCols; col++) {
            createBingoCol(horizontalLayout, col, data.subList(numberOfRows * col, numberOfRows * (col + 1)));
        }
    }

    private void reset(VerticalLayout parent) {
        //UI.getCurrent().getPage().reload();
        parent.remove(this.horizontalLayout);
        createBingoGrid(parent);
        parent.add(this.horizontalLayout);
    }
    ...
    private void bingo() {
        ...
        Button bttnReset = new Button("Reset", event -> {
            reset(this);
            bingo.close();
        });

        bingo.add(h1, image, bttnReset, player);
        bingo.open();
    ...
    protected void onAttach(AttachEvent attachEvent) {
        ...
        broadcasterRegistration = Broadcaster.register(newMessage -> {
            ui.access(() -> showNotification("BINGO! '" + newMessage + "' hat " + count + "x gewonnen!", NotificationVariant.LUMO_SUCCESS));
        });
    }
